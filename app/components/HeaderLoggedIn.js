...
function HeaderLoggedIn(props) {
  ...
  return (
    <div className="flex-row my-3 my-md-0">
      ...
      <ReactToolTip position="bottom" id="search" className="custom=tooltip" />{" "}
      ...
      <ReactToolTip position="bottom" id="chat" className="custom=tooltip" />{" "}
      <Link to={`/profile/${appState.user.username}`} className="mr-2" data-for="profile" data-tip="My Profile">
        <img className="small-header-avatar" src={appState.user.avatar} />
      </Link>
      <ReactToolTip position="bottom" id="profile" className="custom=tooltip" />{" "}
